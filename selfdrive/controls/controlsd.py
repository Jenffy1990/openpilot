...
from openpilot.common.git import get\_short\_branch
from openpilot.common.numpy\_fast import clip
from openpilot.common.params import Params
from openpilot.common.realtime import config\_realtime\_process, Priority, Ratekeeper, DT\_CTRL, DT\_MDL
from openpilot.common.swaglog import cloudlog

from openpilot.selfdrive.car.car\_helpers import get\_car\_interface, get\_startup\_event
from openpilot.selfdrive.car.gm.values import CC\_ONLY\_CAR, GMFlags
from openpilot.selfdrive.controls.lib.alertmanager import AlertManager, set\_offroad\_alert
from openpilot.selfdrive.controls.lib.drive\_helpers import VCruiseHelper, clip\_curvature
from openpilot.selfdrive.controls.lib.events import ET, EVENTS, Events
from openpilot.selfdrive.controls.lib.latcontrol import LatControl, MIN\_LATERAL\_CONTROL\_SPEED
from openpilot.selfdrive.controls.lib.latcontrol\_pid import LatControlPID
from openpilot.selfdrive.controls.lib.latcontrol\_angle import LatControlAngle, STEER\_ANGLE\_SATURATION\_THRESHOLD
from openpilot.selfdrive.controls.lib.latcontrol\_torque import LatControlTorque
from openpilot.selfdrive.controls.lib.longcontrol import LongControl
from openpilot.selfdrive.controls.lib.vehicle\_model import VehicleModel

from openpilot.system.hardware import HARDWARE

from openpilot.selfdrive.frogpilot.controls.lib.frogpilot\_acceleration import get\_max\_allowed\_accel
from openpilot.selfdrive.frogpilot.frogpilot\_variables import ERROR\_LOGS\_PATH, get\_frogpilot\_toggles, params\_memory

SOFT\_DISABLE\_TIME = 3  # seconds
LDW\_MIN\_SPEED = 31 \* CV.MPH\_TO\_MS
LANE\_DEPARTURE\_THRESHOLD = 0.1
CAMERA\_OFFSET = 0.04

REPLAY = "REPLAY" in os.environ
SIMULATION = "SIMULATION" in os.environ
TESTING\_CLOSET = "TESTING\_CLOSET" in os.environ
IGNORE\_PROCESSES = {"loggerd", "encoderd", "statsd"}
...
IGNORED\_SAFETY\_MODES = (SafetyModel.silent, SafetyModel.noOutput)
CSID\_MAP = {"1": EventName.roadCameraError, "2": EventName.wideRoadCameraError, "0": EventName.driverCameraError}
ACTUATOR\_FIELDS = tuple(car.CarControl.Actuators.schema.fields.keys())
ACTIVE\_STATES = (State.enabled, State.softDisabling, State.overriding)
ENABLED\_STATES = (State.preEnabled, \*ACTIVE\_STATES)

class Controls:
def **init**(self, CI=None):
self.params = Params()
...
def set\_initial\_state(self):
if REPLAY:
controls\_state = self.params.get("ReplayControlsState")
if controls\_state is not None:
with log.ControlsState.from\_bytes(controls\_state) as controls\_state:
self.v\_cruise\_helper.v\_cruise\_kph = controls\_state.vCruise
...
def update\_frogpilot\_variables(self, CS):
if self.frogpilot\_toggles.conditional\_experimental\_mode or self.frogpilot\_toggles.slc\_fallback\_experimental\_mode:
self.experimental\_mode = self.sm\['frogpilotPlan'].experimentalMode
...
def publish\_logs(self, CS, start\_time, CC, lac\_log):
...
def read\_personality\_param(self):
try:
return int(self.params.get('LongitudinalPersonality'))
except (ValueError, TypeError):
return log.LongitudinalPersonality.standard

def params\_thread(self, evt):
while not evt.is\_set():
self.is\_metric = self.params.get\_bool("IsMetric")
self.experimental\_mode = self.params.get\_bool("ExperimentalMode") and self.CP.openpilotLongitudinalControl
self.personality = self.read\_personality\_param()
if self.CP.notCar:
self.joystick\_mode = self.params.get\_bool("JoystickDebugMode")
time.sleep(0.1)

def controlsd\_thread(self):
e = threading.Event()
t = threading.Thread(target=self.params\_thread, args=(e, ))
try:
t.start()
while True:
self.step()
self.rk.monitor\_time()
except SystemExit:
e.set()
t.join()

def main():
config\_realtime\_process(4, Priority.CTRL\_HIGH)
controls = Controls()
controls.controlsd\_thread()
